            ...
            <div className="grid gap-3">
              ...
              {isReassignmentEligible(props.order) ? (
                <div className="grid gap-3 rounded-md border bg-muted/20 p-3">
                  {props.options.length > 0 ? <RepresentativeSelect {...props} label="اختيار المندوب الجديد" placeholder="اختر المندوب الجديد" /> : null}
                  <div className="flex flex-wrap justify-end gap-2">
                    <LoadRepresentativesButton loading={props.loading} saving={props.saving} onClick={props.onLoadOptions} />
                    <Button type="button" disabled={!props.selectedId || props.saving} onClick={props.onAssign}>{props.saving ? <Loader2 className="size-4 animate-spin" /> : <Truck className="size-4" />}تغيير المندوب</Button>
                  </div>
                </div>
              ) : null}
              {isReassignmentEligible(props.order) ? <Button type="button" variant="outline" disabled={props.saving} onClick={props.onUnassign}>{props.saving ? <Loader2 className="size-4 animate-spin" /> : <X className="size-4" />}إلغاء إسناد المندوب</Button> : null}
            </div>
          ) : (
            <div className="grid gap-3 rounded-md border bg-muted/20 p-3">
              <div className="text-xs text-muted-foreground">{assignmentReady ? "لم يتم إسناد الطلب لمندوب بعد." : "يجب قبول الطلب قبل إسناده للمندوب."}</div>
              {assignmentReady && props.options.length > 0 ? <RepresentativeSelect {...props} label="اختيار المندوب" placeholder="اختر المندوب" /> : null}
              <div className="flex flex-wrap justify-end gap-2">
...
function LoadRepresentativesButton({ loading, saving, disabled, onClick }: { loading: boolean; saving: boolean; disabled?: boolean; onClick: () => void }) {
  return <Button type="button" variant="outline" disabled={disabled || loading || saving} onClick={onClick}>{loading ? <Loader2 className="size-4 animate-spin" /> : <RefreshCw className="size-4" />}تحديث المندوبين</Button>;
}
...
function AssignedRepresentativeDetails({ order, representatives }: { order: BackendOrder; representatives: Map<string, BackendDashboardUser> }) {
  ...
  return (
    <Link href={representativeHref(order)} aria-label={`عرض تفاصيل المندوب ${name}`} className="mt-2 inline-grid max-w-full gap-1 rounded-md border bg-muted/15 px-3 py-2 text-sm font-semibold text-primary transition hover:border-primary/40 hover:bg-primary/5 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/25">
