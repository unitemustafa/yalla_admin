...
export function MarketVisibilityFields({ form, serviceCitiesLoading, serviceCitiesError, onReloadServiceCities }: {
  ...
  return <>
    ...
    {!form.editing ? <label className="flex min-h-20 cursor-pointer items-center justify-between gap-4 rounded-md border border-primary/25 bg-primary/5 px-4 py-3 shadow-sm transition hover:border-primary/45 sm:col-span-2"><span><span className="block text-sm font-semibold">إرسال إشعار عن المحل</span><span className="mt-1 block text-xs font-normal leading-5 text-muted-foreground">مش هيتبعت فورًا؛ هنستنى أول منتج متاح، وبعدها نبعت إعلان المحل{!cityScope ? " لعملاء جاهز للشحن فقط." : form.draft.selectedServiceCityIds.length ? ` لعملاء ${form.availableServiceCities.find((city) => city.id === form.draft.selectedServiceCityIds[0])?.name ?? "المدينة المحددة"} فقط.` : " لعملاء المدينة اللي هتحددها فقط."} ولو اخترت إشعار المنتج كمان، هنبعت إشعار المحل وحده.</span></span><Switch checked={form.draft.sendStoreNotification} disabled={form.saving} onCheckedChange={(value) => form.update("sendStoreNotification", value)} aria-label="إرسال إشعار عن المحل بعد إضافة أول منتج متاح" /></label> : null}
  </>;
}
