...
export function MarketCategoryFields({ form, classifications }: {
  ...
  return <>
    ...
    <div className="grid gap-3 rounded-lg border p-4 sm:col-span-2">
      <div><h3 className="text-sm font-bold">الفئات الداخلية *</h3><p className="mt-1 text-xs text-muted-foreground">اختر فئات المحل ثم رتّب ظهورها داخل التطبيق.</p></div>
      ...
      })}</div> : <p className="text-xs text-destructive">يجب اختيار فئة واحدة على الأقل.</p>}
    </div>
  </>;
}
