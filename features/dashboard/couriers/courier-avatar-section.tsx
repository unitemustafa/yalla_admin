    ...
    <Card className="flex h-full flex-col overflow-hidden border-border/70 shadow-lg shadow-black/5">
      ...
      <div className="flex flex-1 flex-col px-5 pb-5">
        <div className="grid grid-cols-[auto_minmax(0,1fr)] items-center gap-3 text-start">
          <div className="-mt-12 size-24"><DashboardImage src={form.avatarPreviewUrl || form.draft.avatarUrl} placeholderType="courier" alt="صورة الطيار" width={96} height={96} className="size-24 overflow-hidden rounded-2xl border-4 border-card bg-background shadow-lg" imageClassName="object-cover" /></div>
          <div className="min-w-0"><h3 className="truncate text-lg font-extrabold">{[form.draft.firstName, form.draft.lastName].filter(Boolean).join(" ") || "اسم الطيار"}</h3><p className="mt-1 truncate text-xs text-muted-foreground">{form.draft.phone || "رقم الهاتف سيظهر هنا"}</p></div>
        </div>
