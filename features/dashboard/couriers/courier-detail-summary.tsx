...
export function CourierDetailSummary({ courier, orders, activeOrders, deliveredOrders, deliveredTotal, onReload }: {
  ...
  return <>
    <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center"><div className="flex min-w-0 items-center gap-4"><DashboardImage src={courier.avatar_url} placeholderType="courier" alt={fullNameFromBackendUser(courier)} width={72} height={72} className="size-18 shrink-0 overflow-hidden rounded-full border bg-muted" imageClassName="object-cover" /><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h1 className="truncate text-2xl font-semibold">{fullNameFromBackendUser(courier)}</h1><Badge tone={!hasSignedIn ? "blue" : isAvailable ? "green" : "red"}>{!hasSignedIn ? "لم يسجل الحساب بعد" : isAvailable ? "متاح" : "غير متاح"}</Badge></div><p className="mt-1 text-sm text-muted-foreground">ملف المندوب والطلبات الحالية والسابقة</p></div></div><div className="flex shrink-0 flex-wrap items-center gap-2"><Button type="button" variant="outline" onClick={onReload}><RefreshCw className="size-4" />تحديث</Button><Link href="/delivery/couriers" className="inline-flex h-9 items-center gap-2 rounded-md border px-4 text-sm font-semibold hover:bg-accent"><ArrowRight className="size-4" />الرجوع للمندوبين</Link></div></div>
    <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><SummaryMetric title="الطلبات المسندة" value={orders.length} detail="إجمالي سجل المندوب" icon={<Truck className="size-5" />} /><SummaryMetric title="جاري التسليم" value={activeOrders.length} detail={`${maxActiveOrders || "-"} الحد الأقصى النشط`} icon={<Clock3 className="size-5" />} /><SummaryMetric title="تم التسليم" value={deliveredOrders.length} detail="طلبات تم تسليمها" icon={<CheckCircle2 className="size-5" />} /><SummaryMetric title="قيمة الطلبات المسلمة" value={courierMoney(deliveredTotal)} detail="إجمالي قيمة الطلبات" icon={<CalendarDays className="size-5" />} /></div>
    <div className="mt-6 grid gap-5 xl:grid-cols-[360px_minmax(0,1fr)]"><div className="grid content-start gap-5"><Card className="overflow-hidden"><div className="flex items-center gap-2 border-b bg-muted/25 px-5 py-4 font-semibold"><UserRound className="size-4 text-primary" />بيانات المندوب</div><div className="px-5 py-2"><InfoRow label="اسم المستخدم" value={courier.username} /><InfoRow label="الهاتف" value={courier.phone ? <a href={`tel:${courier.phone}`} className="text-primary hover:underline"><Phone className="me-1 inline size-3.5" />{displayLocalPhone(courier.phone)}</a> : null} dir="ltr" /><InfoRow label="البريد" value={courier.email ? <a href={`mailto:${courier.email}`} className="text-primary hover:underline"><Mail className="me-1 inline size-3.5" />{courier.email}</a> : null} dir="ltr" /><InfoRow label="المركبة" value={profile?.vehicle_type} /><InfoRow label="رقم اللوحة" value={profile?.plate_number} dir="ltr" /><InfoRow label="مدينة التشغيل" value={<><MapPin className="me-1 inline size-3.5 text-primary" />{profile?.service_city_name}</>} /><InfoRow label="التوفر" value={profile?.is_available === false ? "غير متاح" : "متاح"} /><InfoRow label="السعة النشطة" value={`${activeOrders.length} من ${maxActiveOrders || "-"}`} /><InfoRow label="تاريخ الانضمام" value={courierDateTime(courier.created_at)} /></div></Card></div><div className="grid gap-5 md:grid-cols-2"><FeaturedOrder title="الطلب الجاري تسليمه" description="أحدث طلب نشط ومسند للمندوب" order={activeOrders[0]} emptyText="لا يوجد طلب قيد التسليم الآن." /><FeaturedOrder title="آخر طلب تم تسليمه" description="أحدث عملية تم تسليمها" order={deliveredOrders[0]} emptyText="لم يسلم المندوب أي طلب حتى الآن." /></div></div>
  </>;
}
