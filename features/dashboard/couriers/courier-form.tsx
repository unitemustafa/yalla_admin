      ...
      <div className="mx-auto w-full max-w-7xl">
        <header className="mb-7 flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-start gap-4">
            ...
            <div><div className="mb-2 inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/10 px-3 py-1 text-xs font-bold text-primary"><ShieldCheck className="size-3.5" />حساب طيار موثّق</div><h1 className="text-2xl font-black tracking-tight sm:text-3xl">{form.isEditing ? "تعديل بيانات الطيار" : "إضافة طيار جديد"}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">{form.isEditing ? "راجع بيانات الحساب والتوصيل، ثم احفظ التغييرات لتحديث ملف الطيار." : "أنشئ حسابًا متكاملًا للطيار وحدد بيانات مركبته ومنطقة عمله في خطوة واحدة."}</p></div>
          </div>
          <Button type="button" variant="outline" onClick={onClose} className="h-10 self-start rounded-md px-3 lg:self-auto"><ChevronRight className="size-4" />الرجوع إلى الطيارين</Button>
        </header>
