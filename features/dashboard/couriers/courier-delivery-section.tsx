  ...
  return (
    <Card className="overflow-hidden border-border/70 shadow-xl shadow-black/5">
      <section className="border-border/70 bg-muted/15 p-5 sm:p-7 lg:p-8">
        <button type="button" onClick={() => form.setDeliveryOpen((open) => !open)} className="mb-6 flex w-full items-start justify-between gap-3 border-b border-border/70 pb-5 text-start"><span className="flex items-start gap-3"><span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-amber-500/10 text-amber-600 dark:text-amber-400"><Truck className="size-5" /></span><span><span className="block text-lg font-extrabold">بيانات التوصيل</span><span className="mt-1 block text-sm text-muted-foreground">تفاصيل المركبة ونطاق التشغيل الخاص بالمندوب.</span></span></span><ChevronDown className={cn("mt-2 size-5 text-muted-foreground transition-transform", form.deliveryOpen && "rotate-180")} /></button>
