...
export function CourierOrdersTable({ orders, query, onQueryChange }: {
  ...
  return (
    <Card className="mt-6 overflow-hidden">
      <div className="flex flex-col gap-4 border-b p-4 md:flex-row md:items-center md:justify-between"><div><div className="font-semibold">كل طلبات المندوب</div><div className="mt-1 text-xs text-muted-foreground">الطلبات النشطة والمسلمة مع العميل والعنوان والتوقيت</div></div><label className="relative w-full md:w-120"><Search className="pointer-events-none absolute start-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={(event) => onQueryChange(event.target.value)} placeholder="ابحث برقم الطلب أو العميل..." className="h-10 border-border/70 bg-muted/20 ps-9 placeholder:text-muted-foreground/60" /></label></div>
      {orders.length ? (
      ...
      ) : <div className="flex min-h-44 items-center justify-center px-6 py-10 text-center text-sm text-muted-foreground">لا توجد طلبات مطابقة لهذا المندوب.</div>}
    </Card>
  );
}
