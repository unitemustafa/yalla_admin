  ...
  return <div className="px-6 py-8">
    ...
    <div className="mt-6 grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
      <div className="grid gap-5">
        ...
        <FormCard title="المدينة"><Field label="نطاق الظهور"><div className="flex h-9 items-center justify-between rounded-md border px-3"><span>{form.show_in_general ? "عام" : "مدينة واحدة"}</span><Switch checked={!form.show_in_general} onCheckedChange={(checked) => set("show_in_general", !checked)} /></div></Field>{!form.show_in_general ? <SelectField label="مدينة الخدمة" value={form.service_city_id} setValue={(value) => set("service_city_id", value)} options={cities.map((city) => ({ value: String(city.id), label: city.name }))} /> : null}</FormCard>
